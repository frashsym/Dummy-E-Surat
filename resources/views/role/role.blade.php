<x-app-layout>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($roles as $role)
                                    <tr>
                                        <td class="px-6 py-4 text-sm">{{ $roles->firstItem() + $loop->index }}</td>
                                        <td class="px-6 py-4 text-sm">{{ $role->nama_role }}</td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center py-4 text-gray-500 dark:text-gray-400">Belum ada data role.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div id="roleModal"
        ...
        <div id="roleModalContent"
            ...
            <h2 id="modalTitle" class="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-200">Tambah Role</h2>
            <form id="roleForm" method="POST">
                @csrf
                <input type="hidden" name="_method" id="formMethod" value="POST">
                <div class="mb-4">
                    <label for="nama_role" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Role</label>
                    <input type="text" id="modalNamaRole" name="nama_role"
    ...
    <div id="deleteModal"
        ...
        <div id="deleteModalContent"
            ...
            <p class="mb-6 text-gray-600 dark:text-gray-300">Apakah kamu yakin ingin menghapus role ini?</p>
    ...
    <script>
        ...
        const namaInput = document.getElementById('modalNamaRole');

        function openModal(role = null) {
            if (role) {
                modalTitle.textContent = 'Edit Role';
                roleForm.action = `/role/${role.id}`;
                formMethod.value = 'PUT';
                namaInput.value = role.nama_role;
            } else {
        ...
        function closeModal() {
            ...
            setTimeout(() => roleModal.classList.add('hidden'), 200);
        }
